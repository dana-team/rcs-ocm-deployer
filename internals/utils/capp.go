...
// PrepareCappForWorkPayload prepares a Capp resource for inclusion in a manifest work by setting its TypeMeta and ObjectMeta.
func PrepareCappForWorkPayload(capp rcsv1alpha1.Capp) rcsv1alpha1.Capp {
...
func GatherCappResources(capp rcsv1alpha1.Capp, ctx context.Context, l logr.Logger, r client.Client) ([]v1.Manifest, error) {
	...
	svc := PrepareCappForWorkPayload(capp)
...
func RemoveCreatedAnnotation(ctx context.Context, capp rcsv1alpha1.Capp, r client.Client) error {
	cappAnnotations := capp.GetAnnotations()
	delete(cappAnnotations, "AnnotationNamespaceCreated")
	capp.SetAnnotations(cappAnnotations)
