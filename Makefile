...
# dana.io/rcs-ocm-deployer-bundle:$VERSION and dana.io/rcs-ocm-deployer-catalog:$VERSION.
IMAGE_TAG_BASE ?= dana.io/rcs-ocm-deployer
...
ASSERT_LOCATION ?= /usr/local/bin/kubectl-assert
...
install-assert:
	@if ! test -f "$(ASSERT_LOCATION)"; then \
		...
		sudo mv ./kubectl-assert /usr/local/bin; \
	fi
